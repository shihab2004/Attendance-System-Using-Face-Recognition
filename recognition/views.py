...
from .models import Attendence,Student,Faculty
from recognition.custom_dictator import FaceDetector
    ...
    if request.method == 'POST':
        ...
        if Attendence.objects.filter(date = str(date.today()),branch = details['branch'], year = details['year'], section = details['section'],period = details['period']).count() != 0 :
            ...
            if students.exists():
                    ...
                    names = FaceDetector(students)
                    print(names)
                 
            
                    for student in students:
                        if  names and str(student.registration_id) in names:
                    ...
                    _str = ""
                    for i in Attendence.objects.all():
                        _str +=  f"""
                    ...
                    full_str__ = f"""
                    <table>
                        <tr style="background-color: darkmagenta; color:wheat;">
                            <th>Student ID</th>
                            <th>Branch</th>
                            <th>Year</th>
                            <th>Section</th>
                    ...
                    """
                    print(full_str__)
                    path = os.path.join(BASE_DIR,f"PDF/{int(time.time())}")
                    pdfkit.from_string(full_str__, f'{path}.pdf')
                    
                    
                    
                    
                    return render(request,"attendence_table.html", context)
    context = {}
...
def train(request):
    ...
    if request.method == 'POST':
        try:
            ...
            context = {'form':updateStudentForm, 'prev_reg_id':reg_id, 'student':student,"status":True}
        except:
